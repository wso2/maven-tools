...
//        launcher.setApplicationName("org.eclipse.equinox.p2.director");
        ...
        Object result = launcher.run(getDeployRepositoryConfigurations());
    ...
    private String[] getDeployRepositoryConfigurations() {
    	String[] result = new String[] {
    		String.format("-installIU %s", productConfiguration.getId()),
    		String.format("-profile %s", profile),
    		String.format("-bundlepool %s", targetPath.toExternalForm()),
    		//to support shared installation in carbon
            String.format("-shared %s", targetPath.toExternalForm() + File.separator + "p2"),
            //target is set to a separate directory per Profile
    		String.format("-destination %s", targetPath.toExternalForm() + File.separator + profile),
    		String.format("-p2.os %s", "linux"),
    		String.format("-p2.ws %s", "gtk"),
    		String.format("-p2.arch %s", "x86"),
    		"-roaming"
    	};
    	return result; 
    }
