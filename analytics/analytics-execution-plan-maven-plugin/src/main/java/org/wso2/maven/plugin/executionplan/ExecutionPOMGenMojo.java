 ...
 * 
 */
public class ExecutionPOMGenMojo extends AbstractPOMGenMojo {
	...
	private static final String ARTIFACT_TYPE="event/execution-plan";
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		//Mapping Analytics Artifacts to C-App artifacts so that we can reuse the analytics-execution-plan-maven-plugin
	...
	protected void copyResources(MavenProject project, File projectLocation, Artifact artifact)throws IOException {
		...
		File executionplanArtifact = processTokenReplacement(artifact);
		if (executionplanArtifact == null) {
			executionplanArtifact = artifact.getFile();
		}
		FileUtils.copyFile(executionplanArtifact, new File(projectLocation, artifact.getFile().getName()));
		newTag.clearAndEnd();
	}
