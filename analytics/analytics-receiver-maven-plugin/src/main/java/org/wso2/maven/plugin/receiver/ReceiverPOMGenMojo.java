...
package org.wso2.maven.plugin.receiver;
...
/**
 * This is the Maven Mojo used for generating a pom for a receiver artifact 
 * from the old CApp project structure
 * 
 * @goal pom-gen
 * 
 */
public class ReceiverPOMGenMojo extends AbstractPOMGenMojo {
	...
	private static final String ARTIFACT_TYPE="event/receiver";
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		//Mapping Analytics Artifacts to C-App artifacts so that we can reuse the analytics-receiver-maven-plugin
	...
	protected void copyResources(MavenProject project, File projectLocation, Artifact artifact)throws IOException {
		...
		File receiverArtifact = processTokenReplacement(artifact);
		if (receiverArtifact == null) {
			receiverArtifact = artifact.getFile();
		}
		FileUtils.copyFile(receiverArtifact, new File(projectLocation, artifact.getFile().getName()));
		newTag.clearAndEnd();
	}
