...
public class MavenUtils {
    ...
    public static void setupInvoker(Invoker invoker, String projectPath) throws MojoExecutionException {
        ...
        boolean useMavenWrapper = mvnwFile.exists() && mvnwFile.canExecute() && !useLocalMaven(projectPath);
    ...
    public static boolean useLocalMaven(String projectPath) {
        try {
            ...
            if (jsonObject.has(Constants.LOCAL_MVN_SETTING)) {
                return jsonObject.get(Constants.LOCAL_MVN_SETTING).getAsBoolean();
            }
        } catch (Exception e) {
