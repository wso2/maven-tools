 ...
 * 
 */
public class WarPOMGenMojo extends AbstractBundlePOMGenMojo {
	/**
	...
	private static final String ARTIFACT_TYPE="web/application";
...
//	}
	...
	protected MavenProject createMavenProjectForCappArtifact(Artifact artifact,
		...
		if (artifact.getFile().getPath().endsWith(".war")) {
	...
	protected void copyResources(MavenProject project, File projectLocation,
    	...
    	File warFileOriginal=artifact.getFile();
    	File warFileDest=new File(projectLocation+File.separator+warFileOriginal.getName());
    	
    	FileUtils.copyFile(warFileOriginal, warFileDest);
	}
