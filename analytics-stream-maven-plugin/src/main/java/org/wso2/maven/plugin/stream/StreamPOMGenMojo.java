...
package org.wso2.maven.plugin.stream;
...
/**
 * This is the Maven Mojo used for generating a pom for a stream artifact 
 * from the old CApp project structure
 * 
 * @goal pom-gen
 * 
 */
public class StreamPOMGenMojo extends AbstractPOMGenMojo {
	...
	private static final String ARTIFACT_TYPE="event/stream";
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		//Mapping Analytics Artifacts to C-App artifacts so that we can reuse the analytics-stream-maven-plugin
	...
	protected void copyResources(MavenProject project, File projectLocation, Artifact artifact)throws IOException {
		...
		File streamArtifact = processTokenReplacement(artifact);
		if (streamArtifact == null) {
			streamArtifact = artifact.getFile();
		}
		FileUtils.copyFile(streamArtifact, new File(projectLocation, artifact.getFile().getName()));
		newTag.clearAndEnd();
	}
	protected void addPlugins(MavenProject artifactMavenProject, Artifact artifact) {
		Plugin plugin = CAppMavenUtils.createPluginEntry(artifactMavenProject,"org.wso2.maven","analytics-stream-maven-plugin",WSO2MavenAnalyticsPluginConstantants.MAVEN_ANALYTICS_PLUGIN_VERSION,true);
